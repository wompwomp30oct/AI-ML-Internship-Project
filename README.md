...
- `Assignment-2.ipynb` - existing notebook in the folder
...
## Results
Run `Assignment-2.py` after placing the Kaggle CSV in this folder, or let it download the dataset automatically with `kagglehub`. The script prints the evaluation metrics and displays the confusion matrix.

- **Accuracy:** generated when the script is run
- **Precision:** generated when the script is run
- **Recall:** generated when the script is run
- **F1-Score:** generated when the script is run
- **Confusion Matrix:** displayed as a heatmap

## Conclusion
Logistic Regression is a strong baseline for churn prediction because it is simple, fast, and easy to interpret. In this problem, churn is typically influenced by factors such as tenure, contract type, monthly charges, and the set of subscribed services. The model helps identify customers who are more likely to leave, which can support targeted retention efforts. A limitation of Logistic Regression is that it assumes a largely linear relationship between features and the target, so it may miss complex nonlinear patterns and interactions in customer behavior. For that reason, it is useful as a benchmark, but more advanced models may improve churn detection further.
