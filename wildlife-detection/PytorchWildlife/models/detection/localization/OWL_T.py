...
from .model_owl_t import OWLT_Architecture
...
class OWLT(BaseDetector):
    """
    OWL-T (Overhead Wildlife Locator - Transformer) detector class. This class provides utility methods for
    loading the model, generating results, and performing single and batch image detections.
    """

    def __init__(self, weights=None, device="cpu", url="https://zenodo.org/records/18177050/files/HerdNet_Hybrid_Multiscale_Residual_wildme.pth?download=1", transform=None):
        """
        ...
        Args:
            ...
            url (str, optional): 
                URL to fetch the model weights. Defaults to "https://zenodo.org/records/18177050/files/HerdNet_Hybrid_Multiscale_Residual_wildme.pth?download=1".
            transform (torchvision.transforms.Compose, optional):
                Image transformation for inference. Defaults to None.
        """
        super(OWLT, self).__init__(weights=weights, device=device, url=url)
        ...
        self.lmds = HerdNetLMDSLocBranch(**self.lmds_kwargs) # Local Maxima Detection Strategy
    ...
    def _load_model(self, weights=None, device="cpu", url=None):
        ...
        self.CLASS_NAMES = {1: "animal"} # Animal overhead megadetector with single class
        self.img_mean = checkpoint['mean']
        self.img_std = checkpoint['std']

        # Load the model architecture
        self.model = OWLT_Architecture(pretrained_cnn=False)
    ...
    def single_image_detection(self, img, img_path=None, det_conf_thres=0.20, id_strip=None) -> dict:
        ...
        heatmap = preds
        counts, locs, labels, dscores = self.lmds(heatmap)
        preds_array = self.process_lmds_results(counts, locs, labels, dscores, det_conf_thres)
        if img_path:
    ...
    def process_lmds_results(self, counts: list, locs: list, labels: list, dscores: list, det_conf_thres: float = 0.2) -> np.ndarray:
        """
